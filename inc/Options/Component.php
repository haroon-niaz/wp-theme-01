<?php
/**
 * WP_Rig\WP_Rig\Options\Component class
 *
 * @package wp_rig
 */
 ...
 * * `wp_rig()->option()`
 * * `wp_rig()->default()`
 * * `wp_rig()->get_option_type()`
 * * `wp_rig()->get_option_name()`
 * * `wp_rig()->sub_option()`
 * * `wp_rig()->sidebar_options()`
 * * `wp_rig()->palette_option()`
 * * `wp_rig()->get_palette()`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function get_option_type() {
		...
		if ( is_null( self::$opt_type ) ) {
			self::$opt_type = apply_filters( 'wp_rig_theme_option_type', 'theme_mod' );
		}
		// Return option_type.
	...
	public function get_option_name() {
		...
		if ( is_null( self::$opt_name ) ) {
			self::$opt_name = apply_filters( 'wp_rig_theme_option_name', 'wp_rig_settings' );
		}
		// Return option_name.
		...
		if ( is_null( self::$default_options ) ) {
			self::$default_options = apply_filters(
				...
				array(
					...
					// Header HTML.
					'header_html_content'    => __( 'Insert HTML here', 'wp-rig' ),
					...
					// Header Button.
					'header_button_label'      => __( 'Button', 'wp-rig' ),
					'header_button_link'      => '',
					...
					'mobile_button_label'      => __( 'Button', 'wp-rig' ),
					...
					// Mobile Header HTML.
					'mobile_html_content'    => __( 'Insert HTML here', 'wp-rig' ),
					...
					'product_content_element_price' => array(
						'enabled' => true,
						'show_shipping' => false,
						'shipping_statement' => __( '& Free Shipping', 'wp-rig' ),
					),
					...
					'product_content_element_extras' => array(
						'enabled'   => false,
						'title'     => __( 'Free shipping on orders over $50!', 'wp-rig' ),
						'feature_1' => __( 'Satisfaction Guaranteed', 'wp-rig' ),
						'feature_2' => __( 'No Hassle Refunds', 'wp-rig' ),
						'feature_3' => __( 'Secure Payments', 'wp-rig' ),
						'feature_4' => '',
						'feature_5' => '',
					...
					'product_content_element_payments' => array(
						'enabled' => false,
						'title'     => __( 'GUARANTEED SAFE CHECKOUT', 'wp-rig' ),
						'visa' => true,
						'mastercard' => true,
						'amex' => true,
						'discover' => true,
	...
	public function add_default_options() {
		if ( is_null( self::$cpt_options ) ) {
			$add_options = array();
			$all_post_types    = wp_rig()->get_post_types_objects();
			...
			foreach ( $all_post_types as $post_type_item ) {
				...
				$ignore_type     = wp_rig()->get_post_types_to_ignore();
	...
	public function option( $key, $default = '' ) {
		$defaults = self::defaults();
		if ( ! empty( $opt_name_key = apply_filters( 'wp_rig_settings_key_custom_mapping', '', $key ) ) ) {
	...
	public function get_palette_for_customizer() {
		$palette = get_option( 'wp_rig_global_palette' );
	...
	public function palette_option( $subkey, $active_palette = null ) {
		if ( is_null( self::$palette ) ) {
			...
			if ( $palette && ! empty( $palette ) ) {
				...
				self::$palette = json_decode( strval(self::palette_defaults()), true );
			}
		}
		$active = ! empty( $active_palette ) ? $active_palette : apply_filters( 'wp_rig_active_palette', ( self::$palette && is_array( self::$palette ) && isset( self::$palette['active'] ) && ! empty( self::$palette['active'] ) ? self::$palette['active'] : 'palette' ) );
		$value = '';
		...
		return apply_filters( 'wp_rig_palette_option', $value, $subkey );
	}
