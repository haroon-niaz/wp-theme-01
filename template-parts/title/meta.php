...
$elements          = wp_rig()->option( $item_type . '_title_element_meta' );
$title_meta        = array();
$meta_labels       = array();
$meta_divider      = 'dot';
$author_image      = false;
...
if ( isset( $elements ) && is_array( $elements ) ) {
	...
	if ( isset( $elements['author'] ) && $elements['author'] ) {
		...
		if ( isset( $elements['authorEnableLabel'] ) && $elements['authorEnableLabel'] ) {
			$meta_labels['author'] = ( isset( $elements['authorLabel'] ) && ! empty( $elements['authorLabel'] ) ? $elements['authorLabel'] : __( 'By', 'wp-rig' ) );
		}
	...
	if ( isset( $elements['date'] ) && $elements['date'] ) {
		...
		if ( isset( $elements['dateEnableLabel'] ) && $elements['dateEnableLabel'] ) {
			$meta_labels['date'] = ( isset( $elements['dateLabel'] ) && ! empty( $elements['dateLabel'] ) ? $elements['dateLabel'] : __( 'Posted on', 'wp-rig' ) );
		}
	}
	if ( isset( $elements['dateUpdated'] ) && $elements['dateUpdated'] ) {
		...
		if ( isset( $elements['dateUpdatedEnableLabel'] ) && $elements['dateUpdatedEnableLabel'] ) {
			$meta_labels['dateUpdated'] = ( isset( $elements['dateUpdatedLabel'] ) && ! empty( $elements['dateUpdatedLabel'] ) ? $elements['dateUpdatedLabel'] : __( 'Updated on', 'wp-rig' ) );
		}
	}
	if ( isset( $elements['categories'] ) && $elements['categories'] ) {
		...
		if ( isset( $elements['categoriesEnableLabel'] ) && $elements['categoriesEnableLabel'] ) {
			$meta_labels['categories'] = ( isset( $elements['categoriesLabel'] ) && ! empty( $elements['categoriesLabel'] ) ? $elements['categoriesLabel'] : __( 'Posted in', 'wp-rig' ) );
		}
	}
...
<div class="entry-meta entry-meta-divider-<?php echo esc_attr( $meta_divider ); ?>">
	<?php
	do_action( 'wp_rig_before_entry_meta' );
		...
		switch ( $title_meta_item ) {
			case 'author':
				...
				if ( ! empty( $author_string ) ) {
					...
					$author_output .= '</span>';
					echo apply_filters( 'wp_rig_author_meta_output', $author_output ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				break;
			case 'date':
				...
				if ( ! empty( $time_string ) ) {
					...
					<span class="posted-on">
						...
						if ( 'customicon' === $meta_divider ) {
							wp_rig()->print_icon( 'hoursAlt', '', false );
						}
			...
			case 'categories':
				if ( 'post' === get_post_type() ) {
					...
					<span class="category-links">
						<?php
						if ( 'customicon' === $meta_divider ) {
							wp_rig()->print_icon( 'folder', '', false );
						}
				...
				break;
			case 'comments':
				if ( wp_rig()->show_comments() ) {
					echo '<div class="meta-comments">';
					if ( 'customicon' === $meta_divider ) {
						wp_rig()->print_icon( 'commentsAlt', '', false );
					}
					echo '<a class="meta-comments-link anchor-scroll" href="#comments">';
					if ( '1' === get_comments_number() ) {
						echo esc_html( get_comments_number() ) . ' ' . esc_html__( 'Comment', 'wp-rig' );
					} else {
						echo esc_html( get_comments_number() ) . ' ' . esc_html__( 'Comments', 'wp-rig' );
					}
					echo '</a>';
					echo '</div>';
				}
	...
	do_action( 'wp_rig_after_entry_meta' );
	?>
</div><!-- .entry-meta -->
<?php
